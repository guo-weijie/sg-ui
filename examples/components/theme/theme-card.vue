...
<template>
  <section class="theme-card-item" :class="{ 'is-hidden': !config || !config.name, 'is-upload': isUpload }">
    <template v-if="isUpload">
      ...
      <input class="sg-upload__input" type="file" ref="input" @change="uploadAction" accept="application/json" />
    </template>
    <template v-else>
      <div class="preview">
        <div class="line">
        ...
        <div class="action">
          ...
          <div class="action-block">
            <div class="action-item" :class="index && 'action-item-right'" v-for="(item, index) in actionArray"
              :key="index" @click="iconClick(item.action)">
              <div class="icon">
                <img :src="item.icon" />
        ...
        <div class="info-center">
          <div class="title">
            ...
            <span class="right more" v-else>
              <sg-dropdown @command="actionClick">
                <i class="sg-icon-more"></i>
                <sg-dropdown-menu slot="dropdown">
                  <sg-dropdown-item command="rename">{{ getActionDisplayName('rename-theme') }}</sg-dropdown-item>
                  <sg-dropdown-item command="copy">{{ getActionDisplayName('copy-theme') }}</sg-dropdown-item>
                  <sg-dropdown-item command="delete" style="color: #F56C6C;">
...
import dateUtil from 'sg-ui/src/utils/date';
