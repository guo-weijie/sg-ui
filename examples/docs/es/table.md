...
:::demo Después de haber establecido el atributo `data` de `sg-table` con un arreglo de objetos, puede usar la propiedad `prop` (el correspondiente a la clave de un objeto dentro del arreglo `data`) en `sg-table-column` para insertar datos a las columnas de la tabla, y establecer el atributo `label` para definir el nombre de la columna. También puede usar el atributo `width` para establecer el ancho de las columnas.
...
:::demo Utilice `row-class-name` en `sg-table` para agregar clases personalizadas a una fila en específico. Y entonces, podrá darle diseño con estas clases.
```html
<template>
  <sg-table
    :data="tableData"
...
:::demo Al configurar el atributo `height` de `sg-table`, puede fijar la cabecera de la tabla sin agregar otro código.
```html
<template>
  <sg-table
    :data="tableData"
...
:::demo El atributo `fixed` es utilizado en `sg-table-column`, este acepta un `Boolean`. Si es `true`, la columna será fijada a la izquierda. También acepta dos tipos: `left` y `right`, ambos indican donde debe ser fijada la columna.
```html
<template>
  <sg-table
    :data="tableData"
    ...
    <sg-table-column
      ...
      <template slot-scope="scope">
        <sg-button @click="handleClick" type="text" size="small">Detalle</sg-button>
        <sg-button type="text" size="small">Editar</sg-button>
      </template>
    </sg-table-column>
  </sg-table>
</template>
...
:::demo Al configurar el atributo `max-height` de `sg-table`, tu puedes fijar la cabecera de la tabla. La barra de desplazamiento únicamente se mostrará si la altura sobrepasa el valor de la altura máxima.
```html
<template>
  <sg-table
    :data="tableData"
...
:::demo Solo necesitas colocar `sg-table-column` dentro de un `sg-table-column`, de esta forma lograrás agruparlas.
```html
<template>
  <sg-table
    :data="tableData"
    ...
    <sg-table-column label="Información de entrega">
      <sg-table-column
        prop="name"
        label="Nombre"
        width="120">
      </sg-table-column>
      <sg-table-column label="Información de dirección">
        <sg-table-column
          prop="state"
          label="Estado"
          width="120">
...
<template>
  ...
  <div style="margin-top: 20px">
    <sg-button @click="setCurrent(tableData[1])">Seleccionar segunda fila</sg-button>
    <sg-button @click="setCurrent()">Limpiar selección</sg-button>
  </div>
</template>
...
<template>
  ...
  <div style="margin-top: 20px">
    <sg-button @click="toggleSelection([tableData[1], tableData[2]])">Cambia el estado de selección de la segunda y tercera fila.</sg-button>
    <sg-button @click="toggleSelection()">Limpiar selección</sg-button>
  </div>
</template>
...
:::demo Establezca el atributo `filters` y `filter-method` en `sg-table-column` haciendo esta columna filtrable. `filters` es un arreglo, y `filter-method` es una función que decide que filas se muestra. Esta tiene tres parámetros: `value`, `row` y `column`.
```html
<template>
  <sg-button @click="resetDateFilter">清除日期过滤器</sg-button>
  <sg-button @click="clearFilter">清除所有过滤器</sg-button>
  <sg-table
    ref="filterTable"
    :data="tableData"
    style="width: 100%">
  ...
  <sg-table
    ...
    <sg-table-column
      ...
      <template slot-scope="scope">
        <sg-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">Editar</sg-button>
        <sg-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">Eliminar</sg-button>
      </template>
    </sg-table-column>
  </sg-table>
</template>
  ...
  <sg-table
    ...
    <sg-table-column
      ...
      <template slot-scope="scope">
        <sg-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">Edit</sg-button>
        <sg-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">Delete</sg-button>
      </template>
    </sg-table-column>
  </sg-table>
</template>
...
:::demo Puede activar la fila expandible estableciendo la propiedad `type` a `expand` y Scoped Slots. La plantilla para `sg-table-column` se representará como el contenido de la fila expandible, y puede acceder a algunos atributos cuando está usando `Scoped Slots` en plantillas de columna personalizadas.
```html
<template>
  <sg-table
    :data="tableData"
...
| resizable             | especifica si el ancho de la columna puede ser redimensionado, funciona cuando `border` de `sg-table` está en `true` | boolean                           | —                             | false       |
