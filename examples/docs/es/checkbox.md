...
:::demo Define `v-model`(enlaza la variable) en `sg-checkbox`. El valor por defecto es un `Boolean` para un `checkbox`, y se convierte en `true` cuando este es seleccionado. El contenido dentro del tag `sg-checkbox` se convierte en la descripción al costado del botón del checkbox. 

```html
<template>
  <!-- `checked` debe ser true o false -->
  <sg-checkbox v-model="checked">Opción</sg-checkbox>
</template>
<script>
  export default {
    data() {
...
```html
<template>
  <sg-checkbox v-model="checked1" disabled>Opción</sg-checkbox>
  <sg-checkbox v-model="checked2" disabled>Opción</sg-checkbox>
</template>
<script>
  export default {
    data() {
...
:::demo El elemento `checkbox-group` puede manejar múltiples checkboxes en un grupo usando `v-model` el cuál está enlazado a un `Array`. Dentro del elemento `sg-checkbox`, `label` es el valor del checkbox. Si en ese tag no hay contenido anidado, `label` va a ser mostrado como la descripción al lado del botón del checkbox. `label` también se corresponde con los valores del array. Es seleccionado si el valor especificado existe en el array y viceversa.

```html
<template>
  <sg-checkbox-group v-model="checkList">
    <sg-checkbox label="Opción A"></sg-checkbox>
    <sg-checkbox label="Opción B"></sg-checkbox>
    <sg-checkbox label="Opción C"></sg-checkbox>
    <sg-checkbox label="disabled" disabled></sg-checkbox>
    <sg-checkbox label="Seleccionado y deshabilitado" disabled></sg-checkbox>
  </sg-checkbox-group>
</template>
...
```html
<template>
  <sg-checkbox :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange">Marcar todos</sg-checkbox>
...
:::demo Sólo debe cambiar el elemento `sg-checkbox` por el elemento `sg-checkbox-button`. También proveemos el atributo `size`.
```html
<template>
  <div>
...
<template>
  <div>
    <sg-checkbox v-model="checked1" label="Opción1" border></sg-checkbox>
    <sg-checkbox v-model="checked2" label="Opción2" border></sg-checkbox>
  </div>
  <div style="margin-top: 20px">
    <sg-checkbox v-model="checked3" label="Opción1" border size="medium"></sg-checkbox>
    <sg-checkbox v-model="checked4" label="Opción2" border size="medium"></sg-checkbox>
  </div>
  <div style="margin-top: 20px">
    <sg-checkbox-group v-model="checkboxGroup1" size="small">
      <sg-checkbox label="Opción1" border></sg-checkbox>
      <sg-checkbox label="Opción2" border disabled></sg-checkbox>
    </sg-checkbox-group>
  </div>
  <div style="margin-top: 20px">
    <sg-checkbox-group v-model="checkboxGroup2" size="mini" disabled>
      <sg-checkbox label="Opción1" border></sg-checkbox>
      <sg-checkbox label="Opción2" border></sg-checkbox>
    </sg-checkbox-group>
  </div>
</template>
