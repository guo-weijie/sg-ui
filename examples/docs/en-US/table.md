...
:::demo After setting attribute `data` of `sg-table` with an object array, you can use `prop` (corresponding to a key of the object in `data` array) in `sg-table-column` to insert data to table columns, and set the attribute `label` to define the column name. You can also use the attribute `width` to define the width of columns.
...
:::demo Use `row-class-name` in `sg-table` to add custom classes to a certain row. Then you can style it with custom classes.
```html
<template>
  <sg-table
    :data="tableData"
...
:::demo By setting the attribute `height` of `sg-table`, you can fix the table header without any other codes.
```html
<template>
  <sg-table
    :data="tableData"
...
:::demo Attribute `fixed` is used in `sg-table-column`, it accepts a `Boolean`. If `true`, the column will be fixed at left. It also accepts two string literals: 'left' and 'right', both indicating that the column will be fixed at corresponding direction.
```html
<template>
  <sg-table
    :data="tableData"
    ...
    <sg-table-column
      ...
      <template slot-scope="scope">
        ...
        <sg-button type="text" size="small">Edit</sg-button>
      </template>
    </sg-table-column>
  </sg-table>
</template>
...
:::demo  By setting the attribute `max-height` of `sg-table`, you can fix the table header. The table body scrolls only if the height of the rows exceeds the max height value.
```html
<template>
  <sg-table
    :data="tableData"
...
:::demo Only need to place sg-table-column inside a sg-table-column, you can achieve group header.
```html
<template>
  <sg-table
    :data="tableData"
    ...
    <sg-table-column label="Delivery Info">
      <sg-table-column
        prop="name"
        label="Name"
        width="120">
      </sg-table-column>
      <sg-table-column label="Address Info">
        <sg-table-column
          prop="state"
          label="State"
          width="120">
...
<template>
  ...
  <div style="margin-top: 20px">
    <sg-button @click="setCurrent(tableData[1])">Select second row</sg-button>
    <sg-button @click="setCurrent()">Clear selection</sg-button>
  </div>
</template>
...
<template>
  ...
  <div style="margin-top: 20px">
    <sg-button @click="toggleSelection([tableData[1], tableData[2]])">Toggle selection status of second and third rows</sg-button>
    <sg-button @click="toggleSelection()">Clear selection</sg-button>
  </div>
</template>
...
:::demo Set attribute `filters` and `filter-method` in `sg-table-column` makes this column filterable. `filters` is an array, and `filter-method` is a function deciding which rows are displayed. It has three parameters: `value`, `row` and `column`.
```html
<template>
  <sg-button @click="resetDateFilter">reset date filter</sg-button>
  <sg-button @click="clearFilter">reset all filters</sg-button>
  <sg-table
    ref="filterTable"
    :data="tableData"
    style="width: 100%">
...
:::demo Activate expandable row by adding type="expand" and scoped slot. The template for sg-table-column will be rendered as the contents of the expanded row, and you can access the same attributes as when you are using `Scoped slot` in custom column templates.
```html
<template>
  <sg-table
    :data="tableData"
...
:::demo To customize row indices, use `index` attribute on `sg-table-column` with `type=index`. If it is assigned to a number, all indices will have an offset of that number. It also accepts a method with each index (starting from `0`) as parameter, and the returned value will be displayed as index.
...
| resizable | whether column width can be resized, works when `border` of `sg-table` is `true` | boolean | — | false |
