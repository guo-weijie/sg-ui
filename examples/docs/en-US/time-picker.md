...
:::demo Use `sg-time-select` label, then assign start time, end time and time step with `start`, `end` and `step`.
```html
<sg-time-select
  v-model="value"
  :picker-options="{
...
:::demo Use `sg-time-picker` label, and you can limit the time range by specifying `selectableRange`. By default, you can scroll the mouse wheel to pick time, alternatively you can use the control arrows when the `arrow-control` attribute is set.
...
| is-range | whether to pick a time range, only works with `<sg-time-picker>` | boolean | — | false |
| arrow-control | whether to pick time using arrow buttons, only works with `<sg-time-picker>` | boolean | — | false |
...
| prefix-icon | Custom prefix icon class | string | — | sg-icon-time |
| clear-icon | Custom clear icon class | string | — | sg-icon-circle-close |
