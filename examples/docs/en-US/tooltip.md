...
:::demo Override attribute `content` of `sg-tooltip` by adding a slot named `content`.
```html
...
`transition` attribute allows you to customize the animation in which the tooltip shows or hides, and the default value is sg-fade-in-linear.
...
|  transition     |  animation name | string             | — | sg-fade-in-linear |
