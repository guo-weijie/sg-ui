...
| icon-classes | class names of icons. If array, ot should have 3 elements, each of which corresponds with a score level, else if object, the key should be threshold value between two levels, and the value should be corresponding icon class | array/object | — | ['sg-icon-star-on', 'sg-icon-star-on','sg-icon-star-on'] |
| void-icon-class | class name of unselected icons | string | — | sg-icon-star-off |
| disabled-void-icon-class | class name of unselected read-only icons | string | — | sg-icon-star-on |
