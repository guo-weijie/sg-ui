...
:::demo 要使用它，只需要在`sg-input-number`元素中使用`v-model`绑定变量即可，变量的初始值即为默认值。
```html
<template>
  <sg-input-number v-model="num" @change="handleChange" :min="1" :max="10" label="描述文字"></sg-input-number>
</template>
<script>
  export default {
    data() {
