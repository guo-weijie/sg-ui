<template>
  <div ref="reference" :class="[
    ...
    <sg-input ref="input" v-model="multiple ? presentText : inputValue" :size="realSize" :placeholder="placeholder"
      ...
      <template slot="suffix">
        <i v-if="clearBtnVisible" key="clear" class="sg-input__icon sg-icon-circle-close" @click.stop="handleClear"></i>
    ...
    <div v-if="multiple" class="sg-cascader__tags">
      <sg-tag v-for="tag in presentTags" :key="tag.key" type="info" :size="tagSize" :hit="tag.hitState"
        :closable="tag.closable" disable-transitions @close="deleteTag(tag)">
        <span>{{ tag.text }}</span>
      </sg-tag>
      <input v-if="filterable && !isDisabled" v-model.trim="inputValue" type="text" class="sg-cascader__search-input"
    ...
    <transition name="sg-zoom-in-top" @after-leave="handleDropdownLeave">
      <div v-show="dropDownVisible" ref="popper" :class="['sg-popper', 'sg-cascader__dropdown', popperClass]">
        <sg-cascader-panel ref="panel" v-show="!filtering" v-model="checkedValue" :options="options" :props="config"
          :border="false" :render-label="$scopedSlots.default" @expand-change="handleExpandChange"
          @close="toggleDropDownVisible(false)"></sg-cascader-panel>
        <sg-scrollbar ref="suggestionPanel" v-if="filterable" v-show="filtering" tag="ul"
          class="sg-cascader__suggestion-panel" view-class="sg-cascader__suggestion-list"
          @keydown.native="handleSuggestionKeyDown">
          <template v-if="suggestions.length">
            <li v-for="(item, index) in suggestions" :key="item.uid" :class="[
              'sg-cascader__suggestion-item',
              item.checked && 'is-checked'
            ]" :tabindex="-1" @click="handleSuggestionClick(index)">
              <span>{{ item.text }}</span>
              <i v-if="item.checked" class="sg-icon-check"></i>
            </li>
          </template>
          <slot v-else name="empty">
            <li class="sg-cascader__empty-text">{{ t('el.cascader.noMatch') }}</li>
          </slot>
        </sg-scrollbar>
      </div>
    </transition>
...
import SgCascaderPanel from 'sg-ui/packages/cascader-panel';
import AriaUtils from 'sg-ui/src/utils/aria-utils';
import { t } from 'sg-ui/src/locale';
import { isEqual, isEmpty, kebabCase } from 'sg-ui/src/utils/util';
import { isUndefined, isFunction } from 'sg-ui/src/utils/types';
import { isDef } from 'sg-ui/src/utils/shared';
...
export default {
  ...
  computed: {
    realSize() {
      const _sgFormItemSize = (this.sgFormItem || {}).sgFormItemSize;
      return this.size || _sgFormItemSize || (this.$ELEMENT || {}).size;
    },
    tagSize() {
  ...
  methods: {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          firstNode = suggestionPanel.$el.querySelector('.sg-cascader__suggestion-item');
        } else {
          const firstMenu = popper.querySelector('.sg-cascader-menu');
          firstNode = firstMenu.querySelector('.sg-cascader-node[tabindex="-1"]');
        }
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector('.sg-input__inner');

      if (!inputInner) return;

      const tags = $el.querySelector('.sg-cascader__tags');
      let suggestionPanelSg = null;

      if (suggestionPanel && (suggestionPanelSg = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelSg.querySelector('.sg-cascader__suggestion-list');
