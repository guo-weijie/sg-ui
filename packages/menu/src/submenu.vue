...
export default {
  ...
  computed: {
    ...
    menuTransitionName() {
      return this.rootMenu.collapse ? 'sg-zoom-in-left' : 'sg-zoom-in-top';
    },
    opened() {
    ...
    isFirstLevel() {
      ...
      while (parent && parent !== this.rootMenu) {
        if (['SgSubmenu', 'SgMenuItemGroup'].indexOf(parent.$options.componentName) > -1) {
          isFirstLevel = false;
          break;
        } else {
  ...
  methods: {
    ...
    handleClick() {
      ...
      this.dispatch('SgMenu', 'submenu-click', this);
    },
    handleMouseenter(event, showTimeout = this.showTimeout) {
      ...
      this.dispatch('SgSubmenu', 'mouse-enter-child');
    ...
    handleMouseleave(deepDispatch = false) {
      ...
      this.dispatch('SgSubmenu', 'mouse-leave-child');
      clearTimeout(this.timeout);
      this.timeout = setTimeout(() => {
      ...
      if (this.appendToBody && deepDispatch) {
        if (this.$parent.$options.name === 'SgSubmenu') {
  ...
  render(h) {
    ...
    const popupMenu = (
      <transition name={menuTransitionName}>
        <div
          ref="menu"
          v-show={opened}
          class={[`sg-menu--${mode}`, popperClass]}
          ...
          <ul
            role="menu"
            class={['sg-menu sg-menu--popup', `sg-menu--popup-${currentPlacement}`]}
    ...
    ) ? 'sg-icon-arrow-down' : 'sg-icon-arrow-right';
    ...
    return (
      ...
      >
        ...
        >
          {$slots.title}
          <i class={['sg-submenu__icon-arrow', submenuTitleIcon]}></i>
        </div>
