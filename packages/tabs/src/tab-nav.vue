...
export default {
  ...
  render(h) {
    ...
    const scrollBtn = scrollable
      ? [
        <span class={['sg-tabs__nav-prev', scrollable.prev ? '' : 'is-disabled']} on-click={scrollPrev}><i class="sg-icon-arrow-left"></i></span>,
        <span class={['sg-tabs__nav-next', scrollable.next ? '' : 'is-disabled']} on-click={scrollNext}><i class="sg-icon-arrow-right"></i></span>
      ] : null;
    ...
    const tabs = this._l(panes, (pane, index) => {
      ...
      const btnClose = closable
        ? <span class="sg-icon-close" on-click={(ev) => { onTabRemove(pane, ev); }}></span>
        : null;
    ...
    return (
      <div class={['sg-tabs__nav-wrap', scrollable ? 'is-scrollable' : '', `is-${this.rootTabs.tabPosition}`]}>
        {scrollBtn}
        <div class={['sg-tabs__nav-scroll']} ref="navScroll">
          <div
            class={['sg-tabs__nav', `is-${this.rootTabs.tabPosition}`, stretch && ['top', 'bottom'].indexOf(this.rootTabs.tabPosition) !== -1 ? 'is-stretch' : '']}
            ref="nav"
            style={navStyle}
            role="tablist"
