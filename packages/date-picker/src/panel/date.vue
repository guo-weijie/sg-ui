<template>
  <transition name="sg-zoom-in-top" @after-enter="handleEnter" @after-leave="handleLeave">
    <div v-show="visible" class="sg-picker-panel sg-date-picker sg-popper" :class="[{
      'has-sidebar': $slots.sidebar || shortcuts,
      'has-time': showTime
    }, popperClass]">
      <div class="sg-picker-panel__body-wrapper">
        <slot name="sidebar" class="sg-picker-panel__sidebar"></slot>
        <div class="sg-picker-panel__sidebar" v-if="shortcuts">
          <button type="button" class="sg-picker-panel__shortcut" v-for="(shortcut, key) in shortcuts" :key="key"
            @click="handleShortcutClick(shortcut)">{{ shortcut.text }}</button>
        </div>
        <div class="sg-picker-panel__body">
          <div class="sg-date-picker__time-header" v-if="showTime">
            <span class="sg-date-picker__editor-wrap">
              <sg-input :placeholder="t('el.datepicker.selectDate')" :value="visibleDate" size="small"
                @input="val => userInputDate = val" @change="handleVisibleDateChange" />
            </span>
            <span class="sg-date-picker__editor-wrap" v-clickoutside="handleTimePickClose">
              <sg-input ref="input" @focus="timePickerVisible = true" :placeholder="t('el.datepicker.selectTime')"
                :value="visibleTime" size="small" @input="val => userInputTime = val"
                @change="handleVisibleTimeChange" />
              <time-picker ref="timepicker" :time-arrow-control="arrowControl" @pick="handleTimePick"
                :visible="timePickerVisible" @mounted="proxyTimePickerDataProperties">
              </time-picker>
            </span>
          </div>
          <div class="sg-date-picker__header"
            :class="{ 'sg-date-picker__header--bordered': currentView === 'year' || currentView === 'month' }"
            v-show="currentView !== 'time'">
            <button type="button" @click="prevYear" :aria-label="t(`el.datepicker.prevYear`)"
              class="sg-picker-panel__icon-btn sg-date-picker__prev-btn sg-icon-d-arrow-left">
            </button>
            <button type="button" @click="prevMonth" v-show="currentView === 'date'"
              :aria-label="t(`el.datepicker.prevMonth`)"
              class="sg-picker-panel__icon-btn sg-date-picker__prev-btn sg-icon-arrow-left">
            </button>
            <span @click="showYearPicker" role="button" class="sg-date-picker__header-label">{{ yearLabel }}</span>
            <span @click="showMonthPicker" v-show="currentView === 'date'" role="button"
              class="sg-date-picker__header-label"
            ...
            <button type="button" @click="nextYear" :aria-label="t(`el.datepicker.nextYear`)"
              class="sg-picker-panel__icon-btn sg-date-picker__next-btn sg-icon-d-arrow-right">
            </button>
            <button type="button" @click="nextMonth" v-show="currentView === 'date'"
              :aria-label="t(`el.datepicker.nextMonth`)"
              class="sg-picker-panel__icon-btn sg-date-picker__next-btn sg-icon-arrow-right">
            </button>
          </div>

          <div class="sg-picker-panel__content">
            <date-table v-show="currentView === 'date'" @pick="handleDatePick" :selection-mode="selectionMode"
              ...
              :cell-class-name="cellClassName" :disabled-date="disabledDate">
            </date-table>
            <year-table v-show="currentView === 'year'" @pick="handleYearPick" :selection-mode="selectionMode"
              :value="value" :default-value="defaultValue ? new Date(defaultValue) : null" :date="date"
              :disabled-date="disabledDate">
            </year-table>
            <month-table v-show="currentView === 'month'" @pick="handleMonthPick" :selection-mode="selectionMode"
              :value="value" :default-value="defaultValue ? new Date(defaultValue) : null" :date="date"
              :disabled-date="disabledDate">
            </month-table>
          </div>
      ...
      <div class="sg-picker-panel__footer"
        v-show="footerVisible && (currentView === 'date' || currentView === 'month' || currentView === 'year')">
        <sg-button size="mini" type="text" class="sg-picker-panel__link-btn" @click="changeToNow"
          v-show="selectionMode !== 'dates' && selectionMode !== 'months' && selectionMode !== 'years'">
          {{ t('el.datepicker.now') }}
        </sg-button>
        <sg-button plain size="mini" class="sg-picker-panel__link-btn" @click="confirm">
...
import {
  ...
  extractTimeFormat,
  timeWithinRange
} from 'sg-ui/src/utils/date-util';
...
export default {
  ...
  watch: {
    showTime(val) {
      ...
      if (!val) return;
      this.$nextTick(_ => {
        const inputSgm = this.$refs.input.$el;
        if (inputSgm) {
          this.pickerWidth = inputSgm.getBoundingClientRect().width + 10;
        }
      });
    },
  ...
  components: {
    TimePicker, YearTable, MonthTable, DateTable, SgInput, SgButton
  },
