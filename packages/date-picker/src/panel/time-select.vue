<template>
  <transition name="sg-zoom-in-top" @before-enter="handleMenuEnter" @after-leave="$emit('dodestroy')">
    <div ref="popper" v-show="visible" :style="{ width: width + 'px' }" :class="popperClass"
      class="sg-picker-panel time-select sg-popper">
      <sg-scrollbar noresize wrap-class="sg-picker-panel__content">
        <div class="time-select-item" v-for="item in items"
          :class="{ selected: value === item.value, disabled: item.disabled, default: item.value === defaultValue }"
          :disabled="item.disabled" :key="item.value" @click="handleClick(item)">{{ item.value }}</div>
      </sg-scrollbar>
    </div>
  </transition>
</template>
...
export default {
  ...
  methods: {
    ...
    scrollToOption(selector = '.selected') {
      const menu = this.$refs.popper.querySelector('.sg-picker-panel__content');
