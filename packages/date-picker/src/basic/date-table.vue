<template>
  <table cellspacing="0" cellpadding="0" class="sg-date-table" @click="handleClick" @mousemove="handleMouseMove"
    ...
    <tbody>
      ...
      <tr class="sg-date-table__row" v-for="(row, key) in rows" :class="{ current: isWeekActive(row[1]) }" :key="key">
...
<script>
import { getFirstDayOfMonth, getDayCountOfMonth, getWeekNumber, getStartDateOfMonth, prevDate, nextDate, isDate, clearTime as _clearTime } from 'sg-ui/src/utils/date-util';
import Locale from 'sg-ui/src/mixins/locale';
import { arrayFindIndex, arrayFind, coerceTruthyValueToArray } from 'sg-ui/src/utils/util';
